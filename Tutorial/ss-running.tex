...
\tiny
\begin{semiverbatim}
\uncover<1->{\prompt \redcode{cd ~/Cello/cello-src}}\cursor{1}
\uncover<2->{\prompt \redcode{CHARMRUN=~/Charm/charm/bin/charmrun}}\cursor{2}
\uncover<3->{\prompt \redcode{ENZOP=$PWD/bin/enzo-p}}\cursor{3}
\uncover<4->{\prompt \redcode{$CHARMRUN +p4 $ENZOP input/test_implosion.in}}\cursor{4}
\uncover<5->{Charmrun> started all node programs in 1.298 seconds.
